...
ELSE
	BEGIN
		SELECT UserID, UserEmail, Password, FirstName, LastName, Phone, Salt, MembershipLevelID FROM [User] WHERE UserID = @UserID
		IF @@ERROR = 0
			SET @ReturnCode = 0
		ELSE
...
--you could have made @searchTerm and just searched all fields for searchterm
...
CREATE PROCEDURE GetReservationBatchByTimeFrame(@MinDay DateTime, @MaxDay Datetime) AS
DECLARE @ReturnCode INT
SET @ReturnCode = 1
IF @MinDay IS NULL
...
CREATE PROCEDURE GetStandingReservations(@Approved int, @Active int) as
DECLARE @ReturnCode INT
SET @ReturnCode = 1
BEGIN
...
CREATE PROCEDURE GetStandingReservation(@StandingReservationID int) as
DECLARE @ReturnCode INT
SET @ReturnCode = 1
BEGIN
...
BEGIN
	...
	IF @@ERROR = 0
		SET @ReturnCode = 0
	ELSE
		RAISERROR('SelectAllCourses - Select Error at Course Table', 16,1)
END
RETURN @ReturnCode
GO
...
CREATE PROCEDURE UpdateMembershipLevel(@MembershipLevelID int, @Description varchar(50)) AS
DECLARE @ReturnCode INT
SET @ReturnCode = 1
IF @MembershipLevelID IS NULL
...
ELSE
	BEGIN
		UPDATE [MembershipLevel] SET Description = @Description WHERE MembershipLevelID = @MembershipLevelID
		IF @@ERROR = 0
			SET @ReturnCode = 0
		ELSE
